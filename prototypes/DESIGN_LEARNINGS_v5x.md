...
9. CTA panel: dark slate #2C3E50 (permitted dark moment). Structure unchanged: `.section.cta-panel` > `.section-inner.cta-box.reveal` > [verb-first H2] [stage-specific body] [optional 3-card meta strip] [one button]. CTA CONTENT RULES (locked 2026-06-01, EQT-214): (a) Button label is "Get in touch" on EVERY contact CTA across the site, the panel button AND the nav `.nav-cta` AND the hero `.btn-primary`. Never "Book a discovery call" (SaaS funnel signal, wrong for a referral-driven advisory) and never "Let's talk". (b) NO public price ranges. The "Typical investment" meta card is banned site-wide. (c) Meta strip, where used, is exactly three cards: "Typical timeline" | "How we work" = "Embedded with your team" | "What you leave with" = a tangible per-pillar outcome (never the vague "Four deliverables"). ALL SIX service pages carry the strip (FRAME, PLAN, SOLVE, EVOLVE, GOVERN, SCALE) so the range looks consistent. Only Home is card-less: it is not a single pillar, so per-service cards do not map. NOTE: FRAME and PLAN never had the `.cta-meta` CSS in their `<style>` (they shipped card-less); when adding the strip you must also port the `.cta-meta`, `.cta-meta-item`, `.cta-meta-label`, `.cta-meta-value` base rules AND the mobile `@media` override (`.cta-meta { grid-template-columns: 1fr; }`) from a page that has them, or the cards render unstyled. (d) Body copy is firm "we" voice, no named individual (one-man-band signals = procurement risk for corporate/gov buyers), stage-specific per DMAICO step, and must NOT use the old formula "a fifteen-minute conversation will scope X. No obligation." (e) Timeline card anchors LOW: lead with the smallest honest start as "From X weeks", never a wide range and never a month-count at the CTA (a long, scope-dependent range reads as scary at the point of contact and loses new clients). FRAME "From 1 day" (workshop), PLAN "From 2 weeks", SOLVE/EVOLVE/GOVERN/SCALE "From 4 weeks". No month-counts at the CTA. (f) "What you leave with" must be a tangible artefact the buyer receives (e.g. "A costed design and business case", "Control plans and live dashboards"), never a vague end-state ("a team that runs it without us") and never a count ("Four deliverables"). Measurable client numbers belong in case studies, not the CTA. Full copy and the seven ready blocks live in `Runtime/01_Projects/Active/EQUIBT_CTA_REDESIGN_OUTPUT_v1.md`.
