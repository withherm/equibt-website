...
> READ FIRST: the "HARD RULES" and "v5x REVIEW ADDENDUM" sections win over any conflicting rule lower in this file. The "FRAME TEMPLATE SPEC" section immediately below is the locked service-pillar pattern all six service pages inherit. Hero and footer are PURE WHITE #FFFFFF; the footer carries copyright only, no LMI line.
...
3. "Why start" / problem section: light background (off-white #F5F3EE or light grey #F5F5F5), never warm parchment.
4. "What we examine" / disciplines (NOT "phases"): cards, consistent size, on a navy-wash #E8ECF5 section. Cards use `background: var(--navy-wash)`, not white - white cards on a white/off-white section have no contrast.
...
6. Case studies: Problem/Approach/Outcomes, no left-border quote styling, stat stands alone (outcome must not repeat the number), operational metrics only.
...
9. CTA panel: dark slate #2C3E50 (permitted dark moment).
10. Footer: pure white #FFFFFF, dark text, copyright line ONLY. No LMI line. Standard EQUIBT logo.
...
**9. Type scale: cut the contrast between hero H1, section H2, and body.** v5x's hero H1 was far too large relative to H2 and body. Resolve toward restraint: hero H1 52-56px (default 52, weight 600), section H2 36-40px (weight 500), body 18px line-height 1.6. Proportionate steps, not a cliff. Supersedes the earlier 44px H2 / 17px body and the 64-72px hero push.
...
**13. LMI appears in exactly ONE place on the EQUIBT site: the Home page training-handover section.** Not in any footer, not on service pages, not anywhere else. Domain: leanmethodsinstitute.com (global primary), not leanmethods.com.au and not theleanmethods.com. EQUIBT footers carry copyright only, no LMI line. Locked 2026-05-31.
...
3. What we examine: navy-wash #E8ECF5 section, cards `background: var(--navy-wash)` (not white)
...
8. Footer: pure white #FFFFFF, copyright only
...
**Accent colour:** terracotta (#C15F3C) is used sparingly - kicker lines, stat numbers, pillar link text, the one circle in each illustration. Not as a background fill.
...
Still open:
1. Pillar colour at page level: currently only the pill tag. Consider hero section tint or page-level accent. Low priority.
2. Service page hero body text length: "Michael has been here..." paragraphs are long for 2-col layout. May need trimming on desktop view.
