...
**Hero icon update (corrected 2026-07-12):** the interim Forma hero layer is retired. Every hero is Herman's hand-drawn line-art (**stroke art**, navy `#26245C` + one terracotta accent, Safari-flattened), sized `var(--hero-art-w) = min(312px,34vw)`, centred. Delivery is unchanged (SVGs in `HERO_ICON_HANDOFF.json`, thin `Hero*.astro` wrappers). Full set + Resources nav dropdown order in `DESIGN_LEARNINGS_v6x.md` -> "Hero icon set + Resources nav order".
