...
- Site-wide hero art size is locked to `--hero-art-w:min(240px,26vw)` in `tokens.css`; Resources, service pillar, method, industry sector, case-study, insights and interim Forma hero art must reference `var(--hero-art-w)` rather than copying the literal value. Hero art is centred in its own column, with no horizontal translate nudge.
...
- **Hub hero:** a single interim Forma `hero-industry-hub` icon in the standard hero art column, sized by `--hero-art-w`, centred. The old four-sector 2x2 cluster is retired while the EQT-275 bespoke hero scene backlog remains the superseding artwork path.
...
- **Site-wide hero soft orbit (EQT-353):** all page heroes use a one-shot soft-orbit settle via `src/scripts/hero-orbit.ts`, wired once in `src/layouts/Layout.astro`. Motion is X ±8px at the 300px home reference size, scaled by rendered art width with clamp 0.5-1.0; vertical B −2/−3px scaled the same way; rotate stays ±1° fixed; duration is 12s, one loop then rest; reduced-motion is static. Supersedes the EQT-352 home-only note. Motion-only, draw-on remains unwired and compatible because transforms sit on the runtime inner wrapper, not SVG paths.
