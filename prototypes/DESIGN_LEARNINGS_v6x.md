...
**Safari drops a `position:sticky` bar when neighbouring sections repaint — promote it to its own layer (EQT-296).** Both sticky bars (`.sector-subnav`, `.sector-list-sticky`) pinned fine in Chrome but in Safari the subnav detached at a section boundary and never re-pinned (confirmed 2026-06-11; hard failure, not scroll jank — it stayed detached even when stopped). The CSS path was clean (direct child of `<main>`, no ancestor overflow/transform, `.site-nav` sticky works). Cause: WebKit orphans the sticky element's pin when the `industry-reveal` translateY transforms fire on the sections scrolling past it. Chrome composites sticky off-thread and survives; Safari repaints on the main thread and drops it. Fix: give each sticky bar its own compositing layer — `transform:translateZ(0); -webkit-backface-visibility:hidden;` — applied to the bar itself (safe: transform on the sticky element does not disable its own stickiness, only an ancestor transform would). This is the canonical WebKit sticky-repaint fix; prefer it over removing the reveal transforms.
