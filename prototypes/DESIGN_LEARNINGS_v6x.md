...
**Second Safari bug — the hub bar painted a stale ghost even when fixed (EQT-296 follow-up).** After the JS-fixed switch the subnav was correct in Safari but the hub `.sector-list-sticky` still appeared mid-page. Safari's own values proved the layout was right (`position:fixed; top:81px; rectTop:81`, ancestors clean) — it was a compositing/repaint bug: Safari left a stale paint of the fixed bar at an old position because the reveal-animated `.sector-panel` + `.hub-pathway` trainline in the same `.selector-sticky-scope` repaint right next to it. The subnav's scope has no such animated neighbour, so it was fine. First fix attempt: force the hub bar onto its own GPU layer with `transform:translateZ(0); -webkit-backface-visibility:hidden;` on `.sector-list-sticky` so Safari repaints it at its real position. Self-transform does not break the element's own fixed/absolute positioning (only an ancestor transform would). **GPU promotion was NOT sufficient** — the ghost still appeared on a fresh production load in a Safari Private window. Worse, once the animated neighbour was removed the promotion caused its own Safari bug: the composited fixed layer desynced during scroll and the bar rode at the wrong viewport position (e.g. glued to the bottom of the screen). `translateZ(0)`/`backface-visibility` were removed from `.sector-list-sticky`. Do not promote a JS-pinned bar to its own layer — the working `.sector-subnav` is unpromoted.

**Real fix — remove the animated neighbour inside the pin scope (EQT-297).** The actual cure was eliminating the moving paint next to the fixed bar: the `translateY(8px)` reveal on `.industry-head` / `.sector-panel` / `.hub-pathway` inside `.selector-section` was neutralised to `opacity:1; transform:none; transition:none` (both hidden and `.is-visible` states), so nothing inside `.selector-sticky-scope` animates a transform while the bar is pinned. The selector section now appears statically — the lost fade-in on that one section is the accepted trade-off; every other section keeps its reveal. General rule: a JS-fixed bar must not share its pin scope with transform-animated neighbours in Safari — match the subnav, whose scope has no animated neighbour and has never mis-painted.
