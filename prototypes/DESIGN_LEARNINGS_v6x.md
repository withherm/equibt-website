...
- Site-wide hero art size is locked to `--hero-art-w:min(312px,34vw)` in `tokens.css` (**revised 2026-07-12**; the doc previously read `min(240px,26vw)`, the shipped token is `min(312px,34vw)`); Resources, service pillar, method, industry sector, case-study and insights hero art must reference `var(--hero-art-w)` rather than copying the literal value. Hero art is centred in its own column, with no horizontal translate nudge.
...
- **Hero icon weight — SUPERSEDED 2026-07-12.** This bullet described the interim filled-Forma hero icons (EQT-324), now retired. Every hero is Herman's hand-drawn line-art: **stroke art**, navy `#26245C` + one terracotta `#C15F3C` accent, Safari transform-flattened, shipped via `HERO_ICON_HANDOFF.json`. Hero art IS stroke line-art now; the "filled, do not thin-stroke" constraint no longer applies. See the corrected "Hero icon set" section below.
- Interim line-art `hero-motif` art was removed from the Home, About, Capability Statement, Services hub, and Our Work hub heroes. The interim Forma hero icon set now occupies those hero art columns and the Industries, Insights, How We Work, Engagement Model, and Contact hero slots. EQT-275 bespoke hero scenes still supersede this interim set; do not re-add the interim motif (EQT-324 / hero Forma handoff).
...
- **Hub hero:** a single `hero-industry-hub` line-art icon in the standard hero art column, sized by `--hero-art-w`, centred (**2026-07-12**: now hand-drawn line-art, not the interim Forma icon; the EQT-275 supersede path is resolved). The old four-sector 2x2 cluster is retired.
...
**Field guidance + character caps (EQT-332, 2026-06-17).** Each builder field carries an inline `e.g.` example line under the prompt (italic muted, Cowork-authored, corporate-general) — never pre-fill the field itself, it would ride into the downloaded canvas. Each field has a per-field character cap enforced with native `maxlength` (the hard stop is what guarantees the cell fits without resizing) and a live "N left" counter below it (Montserrat, muted, turns terracotta `is-low` at <=20 left). Textareas are `resize:none` with a fixed height. The cap is framed as a feature in the intro lede ("Each field is capped on purpose, so a sharp canvas fits on a single page"). Caps measured to the rendered cell: name ~50, narrative ~150-160, tighter fields ~130. Character caps, not word caps — words vary in width and would not reliably fit the cell.
...
## Hero icon set + Resources nav order (shipped 2026-06-16; hero art re-authored to hand-drawn line-art 2026-07-07)

**UPDATE 2026-07-12 — the interim filled-Forma hero layer is fully retired.** All 30 page heroes (and the 27 functional glyphs) are now Herman's hand-drawn Affinity line-art: stroke art, navy `#26245C` + one terracotta `#C15F3C` accent, flattened for Safari (matrix baked into path coords + scaled stroke-width). The EQT-275 "bespoke hero scenes supersede this" note is resolved: the hand-drawn set IS the destination. The delivery mechanism below still holds (SVGs in `HERO_ICON_HANDOFF.json`, thin `Hero*.astro` wrappers reading it, registered in `Icon.astro`); only the artwork changed from filled Forma to hand-drawn line-art. The Forma catalogue names and the "filled, not stroked" constraint in the bullets below are **historical**, kept for provenance.

Original interim note (historical): interim site-wide hero icons, replacing the EQT-324 stripped / line-art hero art with one filled **Forma Regular** icon per hero.
...
- **Size token.** `--hero-art-w: min(312px,34vw)` in `tokens.css` (**revised 2026-07-12**; was `min(240px,26vw)`, itself scaled up from the EQT-324 `min(210px,22vw)`); every hero icon references the token, never a literal.
...
- **Site-wide hero soft orbit (EQT-353):** all page heroes use a one-shot soft-orbit settle via `src/scripts/hero-orbit.ts`, wired once in `src/layouts/Layout.astro`. Motion is X ±4px reference (scaled by art width, clamp 0.5-1), vertical B −2/−3px scaled, rotate ±1° fixed, 8s, one loop then rest, reduced-motion static. Supersedes the EQT-352 home-only note. Motion-only, draw-on remains unwired and compatible because transforms sit on the runtime inner wrapper, not SVG paths.
