...
**EQT-329 planning and gap checklist.** A checklist can be an output builder without becoming an assessment. The Improvement Programme Checklist uses three item states only: default gap, in place, and not applicable. It deliberately carries no score, count, progress bar, or readiness verdict. N/A rows grey out and are omitted from the takeaway entirely. Owner and note fields ride into the print/PDF output, which lists gaps grouped by phase first and optionally lists items already in place beneath.
