...
**Sticky sector subnav: hide while hero is visible, reveal when sticking.** The `sector-subnav` (CHALLENGES | HOW WE HELP | RESULTS | OUR WORK | WHERE TO START) on sector pages has `position:sticky; top:var(--nav-h)`. Before sticking it sits at its natural document position, which puts it at ~357px (mid-viewport) while the user is scrolling through the hero. This looks like a floating divider bar, not navigation. Fix: `initialiseSubnavVisibility()` in `_sector.astro` uses IntersectionObserver on the hero with `rootMargin:-${navH}px 0px 0px 0px`. While the hero intersects, `.subnav-hidden{opacity:0;pointer-events:none}` is applied. The class is removed exactly when the hero clears the nav height — the same moment the subnav starts sticking at the top. Net effect: the subnav only appears when it is already pinned at the top; it is never visible in a mid-page floating state. JS adds the initial class (so no-JS fallback keeps subnav visible in its natural position).
