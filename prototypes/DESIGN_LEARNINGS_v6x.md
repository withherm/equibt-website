...
**Hub sector selector bar shows only while pinned — deterministic stuck-detector.** Separate from the subnav: the hub `sector-list-sticky` sits deep in the page (after the hero + formula section), so an always-visible in-flow bar there reads as a floating divider. It keeps hide-until-stuck, but via a **deterministic synchronous detector** (the EQT-289 hero IntersectionObserver raced the scroll and flashed the bar mid-page — reproduced live 2026-06-11, `opacity:1` at scrollY 500): `initialiseStickyBarVisibility()` in `index.astro` reads `getBoundingClientRect().top` on each `scroll`/`resize` and shows the bar only when `Math.abs(top - navH) <= 1.5`. Combined with the bounded scope (above), the bar shows across its selector panel and hides cleanly before the closing CTA. `.subnav-hidden{opacity:0;pointer-events:none}` (140ms fade) now applies to `.sector-list-sticky` (the hub bar) as well as `.sector-subnav`.
