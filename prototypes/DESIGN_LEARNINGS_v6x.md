...
- **Hub sector row sticks across its selector panel only (SPEC CHANGE — supersedes "page-long sticky"):** the sticky scope wraps the selector bar + the selector section ONLY, NOT the closing CTA + LMI. Page-long sticky was wrong: it pinned the industry selector over the "Work with us" CTA and the LMI training handover, where a sector picker has no business and reads as a stuck/broken bar. The bar now pins from the selector heading and **releases before the closing CTA**. It is **shown only while actually pinned at the top** (same hide-until-stuck rule as the sector subnav — see below): hidden before it sticks and after it releases, so it never floats mid-page as a white strip over beige. **Gotcha (still true):** `position:sticky` fails silently if any ancestor between the sticky element and the scroll root has `overflow`, `transform`, `filter`, or `will-change`; keep the section-reveal transform on inner content, never the section wrapper. On selection while pinned, scroll the panel to just below the row (respect reduced-motion).
...
**Sticky bars show only while pinned — deterministic stuck-detector (SPEC CHANGE — supersedes the EQT-289 hero IntersectionObserver).** The sector `sector-subnav` and the hub `sector-list-sticky` both have `position:sticky; top:var(--nav-h)`. Before sticking they sit at their natural document position (~357px, mid-viewport), which reads as a floating divider bar, not a pinned nav. The original EQT-289 fix observed the hero with an IntersectionObserver and toggled `.subnav-hidden{opacity:0;pointer-events:none}`. **That observer fired asynchronously and raced the scroll, so the bar flashed visible in its mid-page state** (reproduced live 2026-06-11: at scrollY 500 the subnav read `opacity:1` mid-page). Replaced with a **deterministic, synchronous detector**: on every `scroll`/`resize`, read the bar's own `getBoundingClientRect().top` and show it only when `Math.abs(top - navH) <= 1.5` (i.e. actually stuck). Hidden before it sticks AND after it releases — the latter is what lets the hub bar drop cleanly before its closing CTA. JS adds `.subnav-hidden` on init (no-JS fallback keeps the bars visible in flow). Same helper in both `_sector.astro` (`initialiseSubnavVisibility`) and `index.astro` (`initialiseStickyBarVisibility`); `.subnav-hidden` now covers both `.sector-subnav` and `.sector-list-sticky` with a 140ms opacity fade. Scroll-spy active-state on the subnav still uses its own IntersectionObserver (unaffected).
