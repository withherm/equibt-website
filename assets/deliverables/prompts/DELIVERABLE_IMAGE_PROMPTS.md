...
**Output:** Save as JPG, portrait A4 proportions (210×280mm visible area)
...
Each prompt below produces one photorealistic image of a professional A4 consulting document lying flat on a clean white surface. Generate each image individually. Save with the filename shown above each prompt.

**Consistent system prompt to prepend to every prompt:**

> Photorealistic product shot. A single A4 consulting document lying flat on a clean white marble surface. Soft, even studio lighting with subtle shadows on the left and bottom edges. The document is portrait orientation, white background, visible edge of the page. Professional, minimal, premium look. No hands, no people, no desk clutter. Camera directly overhead, slight 5-degree tilt for depth. Shallow depth of field — document sharp, surface slightly soft.

Then append the specific prompt for each document below.
...
The document shows a professional consulting report cover page. Title: "SPIN Workshop Report" in navy (#26245C). Subtitle: "Situation | Problem | Implication | Need-Payoff". A horizontal colour bar in deep rose (#8B2252) runs below the title. Below the cover header, a partial page-2 visible showing a structured 2×2 analysis grid with handwritten-style annotation marks and typed text blocks. EQUIBT logo mark (small "EQ" monogram) in the bottom right corner. Client name field shows "Confidential" in light grey.
...
The document shows a one-page framework template. Title: "Success Criteria Framework" in navy. Three columns visible: "Metric", "Current Baseline", "Target State". Four rows of data filled in with professional operational metrics (e.g., processing time, error rate, cycle time, customer satisfaction score). A footer row shows a sign-off field and date. Deep rose (#8B2252) column header background. Clean table with thin borders.
...
- Target output resolution: 1240×1754px (A4 at 150dpi) or larger.
