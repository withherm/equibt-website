...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    // IP is read only to pass remoteip to Turnstile. It is never stored or emailed.
    ...
    await env.DB.prepare(
      `INSERT INTO submissions (
        ...
        organisation, problem, user_agent, country
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
      .bind(createdAt, firstName, lastName, email, phone, organisation, problem, userAgent, country)
      .run();
